...
@section('content')
                ...
                <div class="ibox-content">
                    <form action="#" method="post" id="createConfigForm">
                        ...
                        <div class="col-md-12 row" style="margin-bottom: 5px;">
                            <div class="col-md-3">
                                ...
                                <input type="text" placeholder="Configuration Name" name="config_name" id="config_name" class="form-control"
                                       value="{{$result_ary[$config_id]['conf_name']}}">
                            </div>
                            ...
                            <div class="col-md-2" style="margin-left:14px;">
                                ...
                                <select name="year" id="year" class="form-control">
                                    ...
                                    @for($i=2019;$i<=2025;$i++)
                                        <option value="{{$i}}" @if($i==$result_ary[$config_id]['year']){{"selected"}} @endif>{{$i}}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
                        <div class="field_wrapper">
                            @php($counter = 1)
                            @foreach($result_ary[$config_id] as $key=>$val)
                                @if(!in_array($key, ['conf_name', 'year', 'designations', 'designations_by']))
                                    @php($random_number = rand ( 100 , 999 ))
                                    {{--{{dd($random_number)}}--}}
                                    {{--{{dd(explode(',', $val->pe_kpi_questions_id))}}--}}
                                    <div class="col-md-12" style="margin-top: 5px;">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <select name="head_ary[{{$random_number}}]" id="" class="form-control head_dd">
                                                    <option value="">Select Head</option>
                                                    @if(count($heads) > 0)
                                                        @foreach($heads as $info)
                                                            <option value="{{$info->pe_head_titles_id}}"
                                                            @if($val->pe_head_titles_id == $info->pe_head_titles_id){{"selected"}} @endif
                                                            >{{$info->pe_head_titles_name}}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                            </div>
                                            <div class="col-md-2">
                                                <input type="text" placeholder="Weight(%)" value="{{$val->weight}}" name="head_weight[{{$random_number}}]" class="form-control">
                                            </div>
                                            <div class="col-md-3 row">
                                                ...
                                                @else
                                                    <button class="remove_button btn btn-danger" style="margin-left: 4px;">
                                        ...
                                        @if(!empty($val->pe_kpi_questions_id))
                                        <div class="inner-div">
                                            @php($question_ary = explode(',', $val->pe_kpi_questions_id))
                                            @foreach($question_ary as $q)
                                                <div class="row">
                                                    <div class="col-md-5" style="margin-top: 5px;">
                                                        <select name="question_ary[{{$random_number}}][]" id="" class="form-control">
                                                        {{--<select name="question_ary['+val+'][]" id="" class="form-control">$random_number--}}
                                                            <option value="">Select Question</option>
                                                            @if(count($questions) > 0)
                                                                @foreach($questions as $info)
                                                                    <option value="{{$info->pe_kpi_questions_id}}"
                                                                    @if($q == $info->pe_kpi_questions_id){{"selected"}} @endif
                        ...
                        <div class="col-md-12">
                            <button class="btn btn-success updateConfig" style="margin-top: 10px;">Update Config</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            var prev_designation = @json($result_ary[$config_id]['designations']);
            var prev_evaluate_by = @json($result_ary[$config_id]['designations_by']);
            $('#designation').val(prev_designation);
            $('#designation_by').val(prev_evaluate_by);
            // alert(prev_designation);
        ...
        $('.updateConfig').click(function(e){
            ...
            swalConfirm('to Confirm Update Configuration?').then(function (e) {
                if (e.value) {
                    var data_year = $('#year').val();
                    var designation = $('#designation').val();

                    if (data_year.length > 0 && designation.length > 0) {
                        var url = "{{URL::to('pe-update-config')}}";
                        ...
                        makeAjaxPost(data, url).done(function (response) {
                            ...
                            if (response.code == 200) {
                                ...
                                swalRedirect(redirectUrl, 'Configuration Update Successfully', 'success');
                                // }else{
