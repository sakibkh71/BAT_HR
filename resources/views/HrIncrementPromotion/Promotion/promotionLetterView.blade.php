...
<p class="mt-4">Dear Mr./Mrs./Ms <strong> {{$emp_log->name ?? 'N/A'}}</strong>, Congratulations! <br>
    We are very pleased to inform you that you are being promoted to the post of <strong>{{$emp_log->designations_name ??'N/A'}}</strong> </strong> and salary grade in <strong>{{$emp_log->hr_emp_grade_name ??'N/A'}} </strong>  under the company of {{$emp_log->company_name}}  with effect from <strong>{{!empty($emp_log->applicable_date)?toDated($emp_log->applicable_date):'N/A'}}.</strong></p>
...
<p>We are confident that you shall maintain the standard of good performance that you have
    already attained and uphold the zeal and enthusiasm to the highest level. Your participation will enhance {{$emp_log->company_name ?? 'N/A'}} family value and will build a strong team spirit.
</p>
