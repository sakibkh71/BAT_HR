...
<table class="mt-4">
    <tr>
        <td class="pb5 pt5">Name : <strong> Mr./Mrs.  {{$emp_log->name ?? 'N/A'}}</strong></td>
    </tr>
    <tr>
...
<h4 class="mt-4"><strong>Sub: Letter of Promotion.</strong></h4>

<p class="mt-4"> Dear <strong>{{$emp_log->name ?? 'N/A'}}</strong>, <br>
    We are very pleased to inform you that the Management has decided to promoted you as  <strong>{{$emp_log->designations_name ??'N/A'}}</strong>
    under Department of  <strong>{{$emp_log->departments_name ??'N/A'}}</strong> in SR Chemical Industries Limited with effect
    from   <strong>{{!empty($emp_log->applicable_date)?toDated($emp_log->applicable_date):'N/A'}}.</strong>.
</p>
...
<p>We are confident that you shall maintain the standard of good performance that you have already attained and uphold the zeal and enthusiasm to the highest level. Your participation will enhance SR Chemical Industries family value and will build a strong team spirit.</p>
...
<p>
    .................................. <br>
    <strong> Authorize<br>
        SR Chemical Industries Ltd.</strong>
</p>

{{--

<p>Dear Mr./Mrs. <span>{{$emp_log->name}}</span>,</p>
<p>We are please to inform you that you will be receiving an increase in salary grade  <span>{{$emp_log->hr_emp_grade_name_old}}</span> to  <span>{{$emp_log->hr_emp_grade_name_new}}</span> and
    monthly amount increase <span>{{number_format($emp_log->gross_salary,2)}}</span> to  <span>{{number_format($emp_log->min_gross,2)}}</span>.</p>
