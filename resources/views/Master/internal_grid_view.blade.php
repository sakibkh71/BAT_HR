                ...
                <table class="table table-sm table-striped table-bordered table-hover" id="table-to-print">
                    ...
                    @if(!empty($page_data['header']))
                        @foreach ($page_data['header'] as $rownum => $header_row)
                            <tr>
                                @foreach ($header_row as $key => $header_data)
                                    @if(!empty($order_data) && strtolower($header_data['column']) == $order_data[0])
                                        @php($shorter = $key.', "'.$order_data[1].'"')
                                    @endif
...
<script>
    ...
    function generate_datatable(search_data){
        $('#table-to-print').DataTable({
            ...
            "serverSide": true,
            "bProcessing": true,
            <?php  if(!empty($shorter))
                echo '"aaSorting":  [['. $shorter .']],'
            ?>
            "bDestroy": true,
            "language": {
            ...
            'createdRow': function( row, data, dataIndex ) {
                ...
                data[0].map(function (i,k) {
                    if(k === 0){
                        ...
                        if(ids.includes(primary_key_value)){
