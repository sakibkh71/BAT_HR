            ...
            <div class="col-lg-12 no-padding">
                <div class="ibox ">
                    <div class="ibox-title">
                        <h2>Deduction Approval</h2>
                        <div class="ibox-tools">
                                <a class="d-none btn btn-primary btn-xs all_approval" id="all_approve" href="{{url('hr-deduction-bulk-approved')}}">All Approve</a>
                                <a class="d-none btn btn-primary btn-xs single_approval" id="all_approve" href="{{url('hr-deduction-bulk-approved')}}">Approve</a>
                        </div>
                    </div>
                            ...
                            <table class="checkbox-clickable table table-striped table-bordered table-hover dataTables-example">
                                ...
                                @if($approval_list['results'])
                                    @foreach($approval_list['results'] as $item)
                                        <tr id="{{$item->hr_emp_salary_deduction_id}}" class="row-select-toggle" data-code="{{$item->hr_emp_salary_deduction_id}}">
                                            <td align="center">
                                                <input type="checkbox" class="single_approve" data-code="{{$item->hr_emp_salary_deduction_id}}" value="{{$item->hr_emp_salary_deduction_id}}" data-status="{{ $item->deduction_status }}">
                                            </td>
                                            <td>{{toDated($item->deduction_date)}}</td>
                                            <td>{{!empty($item->deduction_amount)?number_format($item->deduction_amount, 2):0}}</td>
            ...
            $(document).on('click','#all_approve',function (e) {               
                ...
                swalConfirm().then(function (e) {
                    if (e.value) {
                        $.ajax({
                            type: 'POST',
                            url: '<?php echo URL::to('hr-deduction-bulk-approved');?>',
