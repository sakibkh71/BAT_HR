                        ...
                        <div class="table-responsive">
                            <table class="checkbox-clickable table table-striped table-bordered table-hover dataTables-example">
                                ...
                                <tr>
                                    ...
                                    <th>Salary Sheet Code </th>
                                    <th>Preparation Date</th>
                                    <th>Salary Sheet Type</th>
                                    <th>Salary Month</th>
                                    <th>Created By</th>
                                    <th>Created At</th>
                                </tr>
                                ...
                                @if($approval_list['results'])
                                    @foreach($approval_list['results'] as $item)
                                        <tr id="{{$item->hr_emp_salary_sheet_code}}" class="row-select-toggle" data-code="{{$item->hr_emp_salary_sheet_code}}">
                                            <td align="center">
                                                <input type="checkbox" class="single_approve" data-code="{{$item->hr_emp_salary_sheet_code}}" value="{{$item->hr_emp_salary_sheet_code}}" data-status="{{ $item->salary_sheet_status }}">
                                            </td>
                                            <td>{{$item->hr_emp_salary_sheet_code}}</td>
                                            <td>{{toDated($item->preparation_date)}}</td>
                                            <td>{{ $item->salary_sheet_type ?? '' }}</td>
                                            <td>{{toDated($item->salary_month)}}</td>
            ...
            $(document).on('click','#all_approve',function (e) {               
                ...
                swalConfirm().then(function (e) {
                    if (e.value) {
                        $.ajax({
                            type: 'POST',
                            url: '<?php echo URL::to('hr-salary-sheet-bulk-approved');?>',
