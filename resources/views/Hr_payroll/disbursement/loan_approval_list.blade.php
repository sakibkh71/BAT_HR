                            ...
                            <table class="checkbox-clickable table table-striped table-bordered table-hover dataTables-example">
                                ...
                                @if($approval_list['results'])
                                    @foreach($approval_list['results'] as $item)
                                        <tr id="{{$item->hr_emp_loan_id}}" class="row-select-toggle" data-code="{{$item->hr_emp_loan_id}}">
                                            <td align="center">
                                                <input type="checkbox" class="single_approve" data-code="{{$item->hr_emp_loan_id}}" value="{{$item->hr_emp_loan_id}}" data-status="{{ $item->loan_status }}">
                                            </td>
                                            <td>{{toDated($item->loan_date)}}</td>
                                            <td>{{$item->loan_type}}</td>
                                            <td>{{!empty($item->loan_amount)?number_format($item->loan_amount, 2):0}}</td>
                                            <td>{{!empty($item->paid_amount)?number_format($item->paid_amount, 2):0}}</td>
                                            <td>{{$item->loan_duration}} Months</td>
                                            <td class="text-right">{{!empty($item->monthly_payment)?number_format($item->monthly_payment, 2):0}}</td>
                                            <td class="text-right">{{!empty($item->due_amount)?number_format($item->due_amount, 2):0}}</td>
            ...
            $(document).on('click','#all_approve',function (e) {               
                ...
                swalConfirm().then(function (e) {
                    if (e.value) {
                        $.ajax({
                            type: 'POST',
                            url: '<?php echo URL::to('hr-loan-bulk-approved');?>',
