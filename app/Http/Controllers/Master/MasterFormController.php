    ...
    public function masterFormDataStore(Request $request) {
       ...
       // dd($insert_data);
        if(isset($request_data['edit_field_key']) && isset($request_data['edit_field_value'])){
            $update_property = DB::table($request->tableName)
                ->where($request_data['edit_field_key'], '=', $request_data['edit_field_value']);
            //->update($insert_data[0]);
            Audit::build($update_property)->update($insert_data[0]);
