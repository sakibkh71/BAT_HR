...
class MasterGridController extends Controller {
    ...
    public function gridPageInfo($gridName){
        ...
        if(!empty($grid_info->sqlorderby)){
            $order_text = strtolower(trim($grid_info->sqlorderby));
            $order_text = str_replace('order by', '', $order_text);
            $order_text = trim(str_replace('`', '', $order_text));
            $order_text = explode(" ",$order_text);
            $orderArr = array_map(function($arg){
                $text = strstr($arg, '.');
                $text = str_replace('.', '', $text);
                return !empty($text) ? $text:$arg;
            },$order_text);

            $data['order_data'] = $orderArr;
        }
    ...
    public function getGridData(Request $request,$type=''){
        ...
        if(!empty($sqlcondition)){
            $sqlcondition = trim(trim($sqlcondition),'AND');
            $grid_info->sqlcondition = !empty($grid_info->sqlcondition)?$grid_info->sqlcondition.' AND '.$sqlcondition:' WHERE '.$sqlcondition;
        }
        ...
        $limit = $req_data['length']=='-1'?1000:$req_data['length']; // Mendatory value
        ...
        $req_data['sqlcondition'] = $grid_info->sqlcondition;
...
//        debug($datatable_condition,1);
        ...
        if($type  == 'excel'){
            $datas =  DB::select($datatable_query);
            $all_data = [];
            $attribute_fields = explode(',', $grid_info->tr_data_attr);
            foreach ($datas as $each){
                $excel_row = [];
                foreach ($each as $name => $td_data) {
    ...
    private function dropDownQueryData($table_header, $data, $start=0, $limit=10, $order=0, $sort='ASC'){
        $order_by = isset($table_header[$order])??$table_header[$order];
        extract($data);
        ...
        if($sqlorderby){
            ...
            $q = DB::select($sql_query.' '.' LIMIT '.$start.', '.$limit);
        }
