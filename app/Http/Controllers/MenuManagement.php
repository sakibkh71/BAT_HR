...
//        self::getSetMenuOnLoad();
    ...
    /*public function getSetMenuOnLoad(){
...
//        debug(Auth::user());
        ...
        $menuquery->select(
            ...
            'sys_menus.parent_menus_id',
            'sys_menus.modules_id',
            'sys_menus.icon_class',
            'sys_menus.menu_url',
            'sys_menus.sort_number'
        );
        $menuquery->where('sys_menus.modules_id', $module_id);
        $menuquery->whereIn('sys_menus.id', $menus);
...
//        debug($menuHtml);
    ...
    public function makeMenuTree($tree = [], $level = 2){
        ...
        foreach ($tree as $key => $val) {
            ...
            if (isset($val->children)) {
                ...
                $html .= '<a href="#">';
                $html .= '<i class="' . $val->icon_class . '"></i> <span class="nav-label">' . $val->name . '</span>';
    ...
    public function menu_list($menu_id = ''){
        ...
        $data['modules'] = DB::select(DB::raw("SELECT id FROM `sys_modules`"));
        foreach ($data['modules'] as $module) {
            $data['menu_list'][$module->id] = $this->getParents($module->id);
        }
...
//        return view('menus.menu_list', compact('data','pageData'));
    ...
    public function get_all_menu($menu_id = NULL, $parent_id = null, $module_id = ''){
        $condition = '';
        if ($parent_id != NULL)
            $condition .= " AND sys_menus.parent_menus_id = $parent_id";
        if ($menu_id != NULL)
            $condition .= " AND sys_menus.sys_users_id = $menu_id";
        if ($module_id != NULL)
            $condition .= " AND sys_menus.modules_id = $module_id";
        $query = DB::select(DB::raw("SELECT sys_menus.* 
            FROM sys_menus 
            INNER JOIN sys_modules ON sys_modules.id = sys_menus.modules_id
            WHERE 1 $condition
    ...
    public function getParents($module_id = ''){
        ...
        foreach ($menu_lists as $key => $menu_list) {
            $child = $this->getChildren($menu_list->id, $module_id);
    ...
    public function getChildren($parent_id = '', $module_id = ''){
        ...
        if (!empty($menu_lists)) {
            ...
            foreach ($menu_lists as $key => $menu_list) {
                $menu_html .= '<li data-jstree=\'"type":"html"}\' data-id="' . $menu_list->id . '">';
                $menu_html .= '<span class="fa ' . $menu_list->icon_class . '"></span>&nbsp;&nbsp;' . $menu_list->name;
                $menu_html .= $this->getChildren($menu_list->id, $module_id);
                $menu_html .= '</li>';
            }
            $menu_html .= '</ul>';
            return $menu_html;
    ...
    public function menu_entry(Request $request){
        $rules = [
            'name' => 'required|unique:sys_menus,name,"' . $request->id . '"',
            'modules_id' => 'required',
            'menu_url' => 'required'
        ];
        ...
        try {
            if ($request->id > 0) {
                ...
                $menu->parent_menus_id = 0;
                $menu->modules_id = $request->modules_id;
