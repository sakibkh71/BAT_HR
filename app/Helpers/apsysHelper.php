...
function __combo($slug = '', $data = array()){
    ...
    $where_clause = isset($where_clause) && !empty($where_clause) ? $where_clause : '';
    if (!empty($slug)) {
        ...
        if (!empty($combodata)) {
            ...
            if(!empty($where_clause)){
                $combodata->sqlcondition = !empty($combodata->sqlcondition)?$combodata->sqlcondition.' '.$where_clause:' WHERE '.$where_clause;
            }
            $session_condition = sessionFilter('sys_dropdowns',$slug,$combodata->sqlcondition);
            if(!empty($session_condition)){
                $combodata->sqlcondition .= !empty($combodata->sqlcondition)?' '.$session_condition:' WHERE '.trim(trim($session_condition),'AND');
            }
//            debug($combodata);
...
function debug2($dt = null, $die = false){
...
function bkp__sessionFilter($event_ref, $slug, $sql){
    $user_access = session()->get('USER_ACCESS');
    $group_access = session()->get('GROUP_ACCESS');
    $PRIVILEGE_POINT = session()->get('PRIVILEGE_POINT');
    $query = '';
...
function sessionFilter($event_ref, $slug, $sql=''){
    $user_access = session()->get('USER_ACCESS');
    $group_access = session()->get('GROUP_ACCESS');
    $level_access = session()->get('USER_LEVELS');
    if(in_array('1',$level_access)){
        return '';
    }
    $query = '';
    if(!empty($group_access)){
        ...
        if(!empty($search_options)){
            ...
            foreach($search_options as $key=>$option){
                ...
                if(($option['permission'] == 'All' && !empty($option['no_permission']))){
                    ...
                    $query .= " AND $key IN (".stringToArray(sessionItemCheck($option['permission'])).")";
                }elseif(!empty($sql_where)){
...
//    $sessionFilter = $query;
//    if($sessionFilter){
//        $sql = trim(trim($sessionFilter),'AND');
//    }
...
function dTime(){
 ...
 //function exportExcel($data_array,$headerArray='',$pre_header_array='',$fileName=''){
function exportExcel($allArr){
    $fileName=app('App\Http\Controllers\QueryToExcel')->generateReport($allArr);
    return $fileName;
}
...
//    $condition_sql = preg_replace('~[\r\n]+~', ' ', $sqlparts['condition_sql']);
    $condition_sql = $sqlparts['condition_sql'];
    ...
    $widget_id = $sqlparts['widget_id'];

    $session_con = (sessionFilter('sys_dashboard','widgetID-'.$widget_id));
    if($session_con){
        $condition_sql .= $session_con;
    }
    $sql = $select_sql . ' ' . $source_sql . ' ' . $condition_sql . ' ' . $having_sql . ' ' . $groupby_sql . ' ' . $orderby_sql . ' ' . $limit_sql;
    return $sql;
}

function pfp_today_achievement($pfp_salary,$pfp_current_achievement){
    $last_day = date('t');
    $today = date('d');
    $achievement_ratio = $pfp_current_achievement*($last_day/$today);
    $achievement_ratio = $achievement_ratio<=110?$achievement_ratio:110;
    return (($achievement_ratio*$pfp_salary)/100);
}